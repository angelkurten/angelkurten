...
```mermaid
graph TD
    A["User Query"] --> B["1. Query Transformation (optional)"]
    B --> B1["Anaphora resolution (if short/vague query)"]
    B --> B2["Query rewrite via LLM (if < 40 chars)"]
    B --> C["2. Hybrid Retrieval (parallel)"]
    C --> C1["ChromaDB Dense (cosine similarity + distance threshold)"]
    C --> C2["BM25 Sparse (keyword matching)"]
    C1 --> D["3. Reciprocal Rank Fusion"]
    C2 --> D
    D --> E["4. Cross-Encoder Reranking"]
    E --> F["5. Hybrid Scoring"]
    F --> F1["60% cross-encoder relevance"]
    F --> F2["20% user feedback"]
    F --> F3["10% recency"]
    F --> F4["10% source type boost"]
    F --> G["6. Source Diversity Filter"]
    G --> H["7. Context Assembly (XML tags)"]
    H --> I["8. LLM Generation (system prompt v2 with citations)"]
    I --> J["Response + Citations + Quality Metrics"]
```
...
![ContextIA before pipeline improvements -- 30% confidence with 155 assumed claims](/images/blog/contextia-before.png)

![ContextIA after pipeline improvements -- 84% confidence with 53 confirmed, 10 inferred, and 89 decision claims](/images/blog/contextia-after.png)
