...
The index lives in memory, rebuilds lazily on first query after ingestion (<1s for 10K chunks), and costs ~5-20MB per tenant. No Elasticsearch, no external services. For our scale, this is the right trade-off.
